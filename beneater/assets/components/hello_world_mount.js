...
vuexStore.watch((s, g) => g['clock/output'], (val) => {
    vuexStore.dispatch('CLK', { rising: val });
});

vuexStore.watch((s, g) => g['memory/multiplexerManual/pin12'], (val) => {
    if (!val) {
        vuexStore.dispatch('WE');
    }
});

vuexStore.watch((s, g) => g['control/resetNand/pin3'], (val) => {
    if (!val) {
        vuexStore.dispatch('C_MICROINSTRUCTION_CLR');
    }
});

vuexStore.watch((s, g) => g['control/resetNand/pin11'], (val) => {
    if (val) {
        vuexStore.dispatch('CLR');
    }
});

vuexStore.watch((s, g) => g['output/andGate/pin3'], (val) => {
    vuexStore.dispatch('CLKOI', { rising: val });
});

vuexStore.watch((s, g) => g['output/counter/pin14'], (val) => {
